...
function SiteNavigation({ path, onNavigate }) {
  const [open, setOpen] = useState(false)
  const navigate = (event, href) => { event.preventDefault(); setOpen(false); onNavigate(href) }
  return <header className="site-nav"><a className="wordmark" href="/" onClick={(event) => navigate(event, '/')}>DAWIT THEODROS <span>/ AF</span></a><button className="menu-toggle" aria-expanded={open} onClick={() => setOpen(!open)}><span /> MENU</button><nav className={open ? 'nav-open' : ''}>{navItems.map(([href, label], index) => <a key={href} className={path === href ? 'active' : ''} href={href} onClick={(event) => navigate(event, href)}><small>0{index + 1}</small>{label}</a>)}</nav></header>
}
...
function App() {
  ...
  return <div className="site-shell"><SiteNavigation path={path} onNavigate={navigate} /><main className="site-main">{content}</main><footer><span>DAWIT THEODROS / ACCOUNTING FIRM</span><span>OLD-SCHOOL PRECISION / MODERN PROFESSIONALISM</span><span>© 2026</span></footer></div>
}
